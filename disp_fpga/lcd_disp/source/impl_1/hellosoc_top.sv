module hellosoc_top(
	...
	wire clk_10khz;
	wire gameClk;
	pll pll_inst(int_osc, tft_clk, clk_10khz); //replaced clk here
	
	clkdiv #(.div(80), .bitSize(7)) gameClk_div(clk_10khz, gameClk); // ~ 120 Hz
	
	*/
	// Let's make our own clock
	...
	*/
	...
	//TODO: set up memory on FPGA for images
	image_buffer ib(.clk_i( ), //TODO: fill this out
        .rst_i( ),
        .clk_en_i( ),
        .wr_en_i( ),
        .wr_data_i( ),
	...
	wire b2;
	wire b3;
	ball #(.X(240), .Y(200), .VX(3), .RADIUS(48)) ball1(x, y, b1, int_osc);
	ball #(.X(120), .Y(130), .VX(-4), .RADIUS(32)) ball2(x, y, b2, int_osc);
	ball #(.X(150), .Y(210), .VX(2), .RADIUS(24)) ball3(x, y, b3, int_osc);
